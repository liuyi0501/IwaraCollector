...
function GetPage(LastPage, d) {
    ...
    ecchi.get('/videos?sort=date&page=' + d).then(response =>{
        ...
        //console.log(pages.indexOf('page='))
            ...
            fs.writeFileSync('./data/www/page/'+d+'.json','{"video":["'+Vid.join('","')+'"],"thumb":["'+  thumb.join('","')+'"],"title":["'+ title.join('","') +'"]}', function (err) {
...
function VideoGet(video, thumb, q) {
    ...
    while (Number(Date.now()) <= Number(second) + 100) { //暂停2秒，防止被墙拉黑
        //等待1秒，什么都不做
    }
    console.log("---------")
...
function GetAllVideo() {
    ...
    axios.get('https://www.iwara.tv/videos?sort=date&page=0', {//获取页面信息
        referer: 'https://www.iwara.tv/',
        ...
        fs.writeFileSync('./data/www/page/'+0+'.json','{"video":["'+Vid.join('","')+'"],"thumb":["'+ thumb.join('","') +'"],"title":["'+ title.join('","') +'"]}', function (err) {
