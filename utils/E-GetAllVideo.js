...
function GetPage(LastPage, d) {
    ...
    ecchi.get('/videos?sort=date&page=' + d).then(response =>{
...
function GetAllVideo() {
    ...
    axios.get('https://ecchi.iwara.tv/videos?sort=date&page=0', {//获取页面信息
        ...
        fs.writeFileSync('./data/ecchi/page/all.json','{"LastPage":"'+PageLast+'"}',function(err){
            if(err)throw(err)
        })
        let video = []
        let thumb = []
        ...
        console.log("第一页提取完成,共计"+video.length+"个视频")
        VideoGet(video, thumb);
        fs.writeFileSync('./data/ecchi/page/'+0+'.json','{"video":["'+Vid.join('","')+'"],"thumb":["'+ thumb.join('","') +'"],"title":["'+ title.join('","') +'"]}', function (err) {
