//这是E-GetAllVideo.js 的sort by likes 版本，用于获取喜欢磅单的单号及视频
const util = require('../utils/common')
var fs = require('fs')
var axios = require('axios');
var cheerio = require('cheerio');
var schedule = require('node-schedule');
var GetVideo = require('../utils/E-GetVideo')
var https = require("https");
var iconv = require("iconv-lite");

var ecchi = axios.create({
...
function GetPage(LastPage, d) {
    ...
    ecchi.get('/videos?sort=likes&page=' + d).then(response =>{
        ...
        //console.log(pages.indexOf('page='))
            ...
            for(var p = 0;p<video.length;p++){
                var start_line = video[p].indexOf('/videos/');
                var id = "";
                for (i = 0; i < video[p].length; i++) {//获取视频的17位ID
                    if (i >= start_line + 8) {
                        id = id + video[p][i];
                    }
                }
            ...
            console.log("获取第"+d+"页完成，共计"+video.length+"条视频")
            VideoGet(video, thumb);//开始爬取视频内容
            fs.writeFileSync('./data/ecchi/like/'+d+'.json','{"video":["'+Vid.join('","')+'"],"thumb":["'+  thumb.join('","')+'"],"title":["'+ title.join('","') +'"]}', function (err) {
...
function VideoGet(video, thumb, q) {
    ...
    console.log("Address is " + "https://ecchi.iwara.tv" + video[q])
    ...
    fs.exists('./data/ecchi/' + id + '.json',function(exist){//检查是否记录过，有则跳过
        if(!exist){
            ecchi(video[q]).then(response =>{
                    if(response.statusText != "OK"){
                        ...
                        GetVideo("https://ecchi.iwara.tv" + video[q], thumb[q], data)//静态存储此视频的数据
                        q++;
...
function GetAllVideo() {
    ...
    axios.get('https://ecchi.iwara.tv/videos?sort=likes&page=0', {//获取页面信息
        header:{
            referer: 'https://ecchi.iwara.tv/',
        ...
        fs.writeFileSync('./data/ecchi/like/all.json','{"LastPage":"'+PageLast+'"}',function(err){
            if(err)throw(err)
        })
        let video = []
        let thumb = []
        ...
        console.log("第一页提取完成,共计"+video.length+"条视频")
        VideoGet(video, thumb);
        fs.writeFileSync('./data/ecchi/like/'+0+'.json','{"video":["'+Vid.join('","')+'"],"thumb":["'+ thumb.join('","') +'"],"title":["'+ title.join('","') +'"]}', function (err) {
