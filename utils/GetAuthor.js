...
const iwara = axios.create({
  baseURL: 'https://www.iwara.tv/users/',
  timeout: 2000,
  headers: {
      Host: 'www.iwara.tv',
      'X-Custom-Header': 'foobar',
      Referer: 'https://www.iwara.tv/users/',
...
/* 用于读取文件列表 */
function makeIndex(){
    var path = './data/author/www/index/'
...
function GetUserList(){
    ...
    fileList.forEach((item)=>{
        item = item.replace('./data/author/www/index/','')
        userList.push(item.replace('.json',''))
    })
    UserDB = {
        uid: userList
    }
    fs.writeFileSync('./data/author/www/list.json',JSON.stringify(UserDB),(err)=>{
        if(err){
            console.log(err);
        }
    })
...
function saveUser(uid,nickname,picture,title,video,thumb,page,allpage,description,JoinDate){
    ...
    fs.writeFileSync('./data/author/www/users/'+uid+'_'+page+'.json',JSON.stringify({
        nickname: nickname,
        picture: picture,
...
function GetUserVideo(num,uid,picture,page,nickname,JoinDate,description,allpage,err){
    iwara.get(encodeURIComponent(nickname)+'/videos?page='+page).then((res)=>{
        ...
        if ( !err )err=0;
        if ( err <= 3 ){
            GetUserVideo(num,uid,picture,page,nickname,JoinDate,description,allpage,err++);
        } else {
            console.log("爬取用户出现三次错误，跳过进行下一位")
            if(Number(num)+1 < userList.length)GetUser(++num);//记录完当前用户后，记录下一用户
            else {
                console.log('全部',userList.length,'均爬取完毕')
                console.log('当前num为',num)
