/*
IwaraCollector Spider v2 --- 爬取视频模板
        data:{
            title: String,
            postDate: string,
            timestamp: int / ms,
...
/*
    ---
    此Key在/utils/v2/user.js 中调用，用以激流用户信息
    authorDataKey
    'iwara:'+site+':video:v2:user:'+author+':data';
    
    ---

    authorKey
    'iwara:'+site+':video:v2:user:'+author;
    
    authorListKey
    'iwara:'+site+':video:v2:userList';
    
    videoKey
    'iwara:'+site+':video:v2:'+id;
    
    videoListKey
    'iwara:'+site+':video:v2:list';
    
    videoLikeKey
    'iwara:'+site+':video:v2:like';
    
    videoWatchKey
    'iwara:'+site+':video:v2:watch';
    
    tagKey
    'iwara:'+site+':video:v2:tag:'+tag[i];
    
    tagListKey
    'iwara:'+site+':video:v2:tagList';
*/
    ...
    */
    ...
    let result = await new Promise((resolve)=>{
        redis.get(videoKey).then((dbData) => {
            ...
            if ( dbData && catchedTime+1000*60*60*24*30 - Date.now() >= 0 && !force){
                resolve({
                err:0,
        ...
        axios.get(encodeURI(url), opt).then((response)=>{
            ...
            redis.zadd(authorKey, id, time);
            redis.zincrby(authorListKey, author, 1);
            
            
            //Like: iwara:www:video:v2:asdasdasdasdasd;
            //存入视频主要数据
            redis.set(videoKey, json);
            
            var videoListKey = 'iwara:'+site+':video:v2:list';
            console.log('videoListKey:',videoListKey);
            //存入视频列表数据
            //Key, member, Num
            redis.zadd(videoListKey, id, time);
            ...
            for( i = 0 ; i < tag.length ; i++){
                ...
                //存入 TAG 数据
                redis.zincrby(tagListKey, tag[i], 1);
                redis.zadd(tagKey, id, time);
